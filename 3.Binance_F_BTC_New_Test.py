...
fee = 0.001  # 바이낸스 선물 수수료(0.05%) 보수적으로 0.2% 적용
...
for coin_data in InvestCoinList:
    ...
    df = GetOhlcv2(binanceX, coin_ticker, '1d', 2022, 2, 17, 0, 0)
    ...
    for i in range(len(df)):
        ...
        if not IsBuy and i > 2 and not IsSellToday:
            ...
            if coin_ticker == 'DOGE/USDT':
                ...
                if  df['50ma'].iloc[i-2] < df['50ma'].iloc[i-1] and df['open'].iloc[i-1] < df['close'].iloc[i-1] and df['open'].iloc[i-2] < df['close'].iloc[i-2] and df['close'].iloc[i-2] < df['close'].iloc[i-1] and df['high'].iloc[i-2] < df['high'].iloc[i-1] and df['7ma'].iloc[i-2] < df['7ma'].iloc[i-1] and df['16ma'].iloc[i-1] < df['close'].iloc[i-1] and df['73ma'].iloc[i-1] < df['close'].iloc[i-1] and df['30ma_slope'].iloc[i-1] > DiffValue and df['rsi_5ma'].iloc[i] > df['rsi_5ma'].iloc[i-1]:
            ...
            if IsMaDone:
                Rate = 1.0
                InvestGoMoney = RemainInvestMoney * Rate * (1.0 - (fee*leverage))  # 수정: 매수 시 레버리지 미적용
                IsBuyGo = True

            if IsBuyGo:
                if InvestGoMoney > df['value_ma'].iloc[i-1] / 100:
