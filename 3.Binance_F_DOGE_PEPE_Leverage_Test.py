...
current_cycle_investment_base = InvestTotalMoney  # [수정] 사이클 투자 기준금 변수 추가
...
for month_key, count in MonthlyTryCnt.items():
    ...
    yearly_trades[year_key] = yearly_trades.get(year_key, 0) + count
...
# 코인별 익절/손절 통계 출력
print("\n---------- 코인별 거래 통계 ----------")
for ticker in [cd['ticker'] for cd in InvestCoinList]:
    stats = CoinStats[ticker]
...
axs[1].plot(result_df.index, result_df['MaxDrawdown'] * 100, color='red', label='MDD')
axs[1].fill_between(result_df.index, result_df['Drawdown'] * 100, 0, color='red', alpha=0.3, label='Drawdown')
