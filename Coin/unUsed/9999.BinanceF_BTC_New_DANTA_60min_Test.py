...
fee = 0.0005  # 바이낸스 선물 수수료(0.02%) 적용
...
for coin_data in InvestCoinList:
    ...
    df['rsi_ma'] = df['rsi'].rolling(8).mean()  # 약 8시간
    df['rsi_5ma'] = df['rsi'].rolling(4).mean()  # 약 4시간
    df['prev_close'] = df['close'].shift(1)
    df['change'] = (df['close'] - df['prev_close']) / df['prev_close']
    df['value'] = df['close'] * df['volume']

    # 이동평균선 계산 (1시간봉에 맞게 범위 조정)
    ma_dfs = []
    for ma in range(3, 41):  # 최대 약 40시간
        ma_df = df['close'].rolling(ma).mean().rename(str(ma) + 'ma')
        ma_dfs.append(ma_df)
    ma_df_combined = pd.concat(ma_dfs, axis=1)
    df = pd.concat([df, ma_df_combined], axis=1)
    df['value_ma'] = df['value'].rolling(window=8).mean().shift(1)  # 8 캔들
    df['30ma_slope'] = ((df['30ma'] - df['30ma'].shift(4)) / df['30ma'].shift(4)) * 100  # 4 캔들
    DiffValue = -0.5  # 1시간봉에 맞게 조정
    ...
    for i in range(1, len(df)):  # i=0을 건너뛰고 1부터 시작
        ...
        if IsBuy:
            ...
            if InvestMoney < InvestTotalMoney / 3:
                ...
                TotalMoneyList.append(InvestMoney)  # 한 번만 추가
                continue
        ...
        if not IsBuy and i > 2 and not IsSellToday:
            ...
            if coin_ticker == 'POPCAT/USDT':
                ...
                if df['open'].iloc[i-1] < df['close'].iloc[i-1] and df['open'].iloc[i-2] < df['close'].iloc[i-2] and \
                   ...
                   df['40ma'].iloc[i-1] < df['close'].iloc[i-1] and df['30ma_slope'].iloc[i-1] > DiffValue and \
        ...
        # 한 번만 append하도록 보장
        if len(TotalMoneyList) < i + 1:  # i=1부터 시작하므로 +1
            InvestMoney = RealInvestMoney + RemainInvestMoney
...
if len(ResultList) > 0:
    ...
    axs[0].plot(result_df['Cum_Ror'] * 100, label='Strategy (5x Leverage, 1h)')
