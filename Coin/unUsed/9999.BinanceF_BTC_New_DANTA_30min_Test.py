...
fee = 0.0005  # 바이낸스 선물 수수료(0.05%) 0.02% 적용
...
for coin_data in InvestCoinList:
    ...
    # 30분봉 데이터 가져오기
    df = GetOhlcv2(binanceX, coin_ticker, '30m', 2022, 1, 1, 0, 0)
    df = df[df.index < '2025-04-30']  # 데이터 기간 제한
    print(f"Data length: {len(df)}, Start: {df.index[0]}, End: {df.index[-1]}")  # 디버깅: 데이터 범위 출력

    # RSI 지표 계산 (30분봉에 맞게 조정)
    period = 14  # 14 캔들 (약 420분, 7시간)
    delta = df["close"].diff()
    ...
    df['rsi_ma'] = df['rsi'].rolling(10).mean()  # 약 5시간
    df['rsi_5ma'] = df['rsi'].rolling(5).mean()  # 약 2.5시간
    df['prev_close'] = df['close'].shift(1)
    df['change'] = (df['close'] - df['prev_close']) / df['prev_close']
    df['value'] = df['close'] * df['volume']

    # 이동평균선 계산 (30분봉에 맞게 범위 조정)
    ma_dfs = []
    for ma in range(3, 61):  # 최대 약 30시간
        ma_df = df['close'].rolling(ma).mean().rename(str(ma) + 'ma')
        ma_dfs.append(ma_df)
    ma_df_combined = pd.concat(ma_dfs, axis=1)
    df = pd.concat([df, ma_df_combined], axis=1)
    df['value_ma'] = df['value'].rolling(window=10).mean().shift(1)  # 10 캔들 (300분)
    df['30ma_slope'] = ((df['30ma'] - df['30ma'].shift(5)) / df['30ma'].shift(5)) * 100  # 30 캔들 (900분)
    DiffValue = -0.6  # 30분봉에 맞게 조정 (기존 -0.8에서 완화)
    ...
    FirstDateIndex = 0
    TotalMoneyList = []
    ma1, ma2, ma3 = 3, 12, 24  # 90, 360, 720분
    BUY_PRICE = 0
    IsDolpaDay = False
        ...
        if not IsBuy and i > 2 and not IsSellToday:
            ...
            if coin_ticker == 'BTC/USDT':
                ...
                if df['open'].iloc[i-1] < df['close'].iloc[i-1] and df['open'].iloc[i-2] < df['close'].iloc[i-2] and \
                   ...
                   df['53ma'].iloc[i-1] < df['close'].iloc[i-1] and df['30ma_slope'].iloc[i-1] > DiffValue and \
...
if len(ResultList) > 0:
    ...
    axs[0].plot(result_df['Cum_Ror'] * 100, label='Strategy (5x Leverage, 30m)')  # 레이블 수정
