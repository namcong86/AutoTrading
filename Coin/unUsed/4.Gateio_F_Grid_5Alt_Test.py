...
InvestCoinList = [
    {'ticker': 'ETH/USDT:USDT', 'rate': 0.2},
    {'ticker': 'XRP/USDT:USDT', 'rate': 0.2},
    {'ticker': 'SOL/USDT:USDT', 'rate': 0.2},
    {'ticker': 'ADA/USDT:USDT', 'rate': 0.2},
    {'ticker': 'DOGE/USDT:USDT', 'rate': 0.2}
]
...
for coin_data in InvestCoinList:
    ...
    csv_file = 'btc_usdt_1d_2020_to_202504.csv'
    df = load_and_fetch_ohlcv(gateio, coin_ticker, '1d', csv_file, from_date, to_date)
...
if len(ResultList) > 0:
    ...
    # Subplot 1: BTC/USDT Price
    axs[0].plot(df.index, df['close'], label='BTC/USDT Close Price', color='blue')
    axs[0].set_title('BTC/USDT Price (Daily)')
    axs[0].set_ylabel('Price (USDT)')
    axs[0].grid(True)
    axs[0].legend()
    
    # Subplot 2: Cumulative Return
    axs[1].plot(result_df.index, result_df['Cum_Ror'] * 100, label='Strategy (5x Leverage, 1d)', color='green')
