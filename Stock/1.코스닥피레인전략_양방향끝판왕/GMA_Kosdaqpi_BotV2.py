...
https://blog.naver.com/zacra/223341995871
V2는 위 포스팅 참고하세요!!
...
if IsMarketOpen == True and IsLP_OK == True: 
    ...
    for stock_code in InvestStockList:
        ...
        df['ma5_before2'] = df['close'].rolling(5).mean().shift(1)
        ...
        if weekday == time_info.tm_wday:
            ...
            for stock_code in  all_stocks.index:
                ...
                if stock_code in ["233740","251340"]:
                    ...
                    # V2의 변경 추가 사항!!!!!
                    if stock_data['ma20_before'].values[0] < PrevClosePrice and stock_data['ma20_before'].values[0] < stock_data['ma5_before'].values[0] and stock_data['ma5_before2'].values[0] < stock_data['ma5_before'].values[0] and stock_data['ma20_before2'].values[0] < stock_data['ma20_before'].values[0] :
        
                        DolPaPrice = stock_data['low'].values[0] + ((stock_data['prevHigh'].values[0] - stock_data['prevLow'].values[0]) * DolpaRate)

                        if stock_code == "251340":
                            DolPaPrice = stock_data['low'].values[0] + ((max(stock_data['prevHigh'].values[0],stock_data['prevHigh2'].values[0])- min(stock_data['prevLow'].values[0],stock_data['prevLow2'].values[0])) * DolpaRate)
    ...
    if day_str == DateData['Date']: #오늘 할일을 한다!
        ...
        for KospidaqStrategyData in KospidaqStrategyList:
            ...
            if len(stock_data) == 1:
                ...
                ################## V2의 변경 추가 사항!!!!! ##############################
                ########################################################################
                if KospidaqStrategyData['DayStatus'] == "BUY_DONE_DAY":
                    ...
                    if KospidaqStrategyData['Status'] == "INVESTING": #투자중 상태
                        ...
                        if stock_code in ["233740","251340"]:
                            ...
                            if stock_amt > 0:
                                ...
                                CutPrice = stock_data['high'].values[0] - ((stock_data['prevHigh'].values[0] - stock_data['prevLow'].values[0]) * CutRate)

                            

                                CurrentPrice = KisKR.GetCurrentPrice(stock_code)  

                                print("--Check--- NowPrice:", CurrentPrice, " LowPrice:" ,stock_data['low'].values[0], " CutPrice:", CutPrice)

                                if CurrentPrice <= CutPrice :
                                    
                                    
                                    if ENABLE_ORDER_EXECUTION == True:
                                        pprint.pprint(KisKR.MakeSellMarketOrder(stock_code,int(stock_amt*0.5)))
                                    

                                        msg = KospidaqStrategyData['StockName']  + " 일부 시장가 매도!!! " + str(stock_revenue_money) + " 일부 수익 확정!! 수익률:" + str(stock_revenue_rate) + "%"
                                        print(msg)
                                        line_alert.SendMessage(msg)
                ...
                if KospidaqStrategyData['DayStatus'] == "BUY_DAY":              
                    ...
                    if KospidaqStrategyData['Status'] == "INVESTING_TRY":
                        ...
                        if stock_amt > 0:
                            ...
                            # V2의 변경 추가 사항!!!!!
                            if stock_code in ["233740","251340"]:
                                KospidaqStrategyData['DayStatus'] = "BUY_DONE_DAY"
                            else:
                                KospidaqStrategyData['DayStatus'] = "NONE"
