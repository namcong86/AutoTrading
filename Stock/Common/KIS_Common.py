...
def MakeToken(dist = "REAL"):
    ...
    if res.status_code == 200:
        ...
        # 토큰 만료 시간 계산 (발급 후 24시간)
        expire_time = datetime.now() + timedelta(hours=24)

        #빈 딕셔너리를 선언합니다!
        dataDict = dict()

        #해당 토큰과 만료 시간을 파일로 저장해 둡니다!
        dataDict["authorization"] = my_token
        dataDict["expire_time"] = expire_time.isoformat()  # ISO 형식으로 저장
        with open(GetTokenPath(dist), 'w') as outfile:
            json.dump(dataDict, outfile)   

        print("TOKEN : ", my_token)
        print(f"토큰 만료 시간: {expire_time.strftime('%Y-%m-%d %H:%M:%S')}")
...
def GetToken(dist = "REAL"):
    ...
    try:
        ...
        # 토큰 만료 시간 체크 (저장된 경우)
        if 'expire_time' in dataDict:
            expire_time = datetime.fromisoformat(dataDict['expire_time'])
            now = datetime.now()
            
            # 토큰 만료 10분 전에 미리 갱신
            if now >= expire_time - timedelta(minutes=10):
                print(f"토큰이 만료되었거나 곧 만료됩니다. 새 토큰을 발급받습니다. ({dist})")
...
############################################################################################################################################################
#토큰 만료 체크 및 자동 갱신 데코레이터
def auto_refresh_token(func):
    """API 호출 시 토큰 만료 에러를 자동으로 처리하는 데코레이터"""
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 응답이 딕셔너리이고 에러 코드가 있는 경우 체크
                if isinstance(result, dict):
                    # 토큰 만료 에러 체크
                    if result.get('rt_cd') == '1' and '기간이 만료된 token' in str(result.get('msg1', '')):
                        print(f"토큰 만료 감지! 새 토큰을 발급받습니다. (시도 {attempt + 1}/{max_retries})")
                        MakeToken(NOW_DIST)
                        continue  # 재시도
                    # msg_cd가 있는 에러 응답 체크
                    elif result.get('msg_cd') == 'EGW00123':
                        print(f"토큰 만료 감지! 새 토큰을 발급받습니다. (시도 {attempt + 1}/{max_retries})")
                        MakeToken(NOW_DIST)
                        continue  # 재시도
                ...
                # 토큰 관련 예외 처리
                if '기간이 만료된 token' in str(e) or 'EGW00123' in str(e):
...
def GetOhlcv2(area, stock_code, limit = 500, adj_ok = "1"):
    ...
    if area == "KR":
        ...
        df = stock.get_market_ohlcv_by_date(GetFromNowDateStr(area,"BAR",-limit), GetNowDateStr(area,"BAR"), stock_code)
        df.columns = ['Open', 'High', 'Low', 'Close', 'Volume']

    else:
        raise Exception("yfinance module not installed. Only KR area supported.")
