...
if pcServerGb == "congsnas":
    botdata_file_path = "/var/services/homes/AutoBot/BinanceF_15m_Data.json"
else:
    botdata_file_path = "C:\\AutoTrading\\AutoTrading\\BinanceF_15m_Data.json"
...
for coin_data in InvestCoinList:
    ...
    if abs(amt_b) > 0:
        ...
        if BotDataDict[coin_ticker + "_DATE_CHECK"] != day_n:
            msg = f"3.바이낸스 15분 단타봇 현재수익률: ~{round(revenue_rate_b * set_leverage, 2)}% 수익금: ~{format(round(unrealizedProfit_b), ',')} USDT"
            logger.info(msg)
            telegram_alert.SendMessage(msg)
        ...
        if IsSellGo:
            ...
            msg = f"{coin_ticker} 매도: {'Forced Liquidation' if IsDolpaCut else 'Condition Not Met'}! 수익률: ~{round(revenue_rate_b * set_leverage, 2)}% 수익금: ~{format(round(unrealizedProfit_b), ',')} USDT"
            logger.info(msg)
            telegram_alert.SendMessage(msg)
    ...
    # 잔고가 없는 경우 (매수 로직)
        ...
        if (df['open'].iloc[-2] < df['close'].iloc[-2] and
            ...
            if IsMaDone:
                ...
                msg = f"{coin_ticker} 포지션롱 진입! => 수량: {Buy_Amt} 가격: {BUY_PRICE}"
                logger.info(msg)
                telegram_alert.SendMessage(msg)
