...
# 레버리지 설정
LEVERAGE = 3  # 3배 레버리지
LEVERAGE_FEE = 0.0004  # 레버리지 수수료 (일반 수수료보다 높게 설정)
MARGIN_CALL_RATE = 0.8  # 마진콜 기준 (자산의 80% 손실 시 청산)

# 레버리지 적용된 초기 투자금
InvestTotalMoney = InvestTotalMoney * LEVERAGE
    ...
    for i in range(len(df)):
        ...
        if IsBuy == False and i > 2 and IsSellToday == False: 
            ...
            if IsMaDone == True  :
                ...
                # 레버리지 수수료 적용
                daily_fee = RealInvestMoney * LEVERAGE_FEE
                RealInvestMoney -= daily_fee

                # 마진콜 체크
                if RealInvestMoney < (InvestTotalMoney * (1 - MARGIN_CALL_RATE)):
                    IsSellGo = True
                    print("마진콜 발생! 강제 청산")

                InvestGoMoney = RemainInvestMoney*Rate * (1.0 - LEVERAGE_FEE) #레버리지 수수료를 제외한 금액을 투자한다!
                IsBuyGo = True
            ...
            if IsBuyGo == True :
                ...
                #투자금 거래대금 10일 평균의 1/2000수준으로 제한! (레버리지 고려)
                if InvestGoMoney > df['value_ma'].iloc[i-1] / 1000 * LEVERAGE:
                    InvestGoMoney = df['value_ma'].iloc[i-1]/ 1000 * LEVERAGE
                ...
                NowFee = (BuyAmt*BUY_PRICE) * LEVERAGE_FEE
...
if len(ResultList) > 0:
    ...
    print("레버리지 3배 적용 CAGR:", round(CAGR * LEVERAGE,2), "%")
