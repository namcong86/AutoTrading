...
fee = 0.002  # 바이낸스 선물 수수료(0.05%) 보수적으로 0.2% 적용
...
for coin_data in InvestCoinList:
    ...
    df = GetOhlcv2(binanceX, coin_ticker, '1d', 2021, 10, 1, 0, 0)
    df = df[df.index < '2025-04-25']  # 데이터 기간 제한 (2022-02-28까지)
    ...
    for i in range(len(df)):
        ...
        if IsBuy:
            ...
            if InvestMoney < InvestTotalMoney / 9:
        ...
        if not IsBuy and i > 2 and not IsSellToday:
            ...
            if IsMaDone:
                Rate = 1.0 
                # 50이평이 감소하거나 종가가 50이평 밑에 있으면 감액
...
if len(ResultList) > 0:
    ...
    axs[0].plot(result_df['Cum_Ror'] * 100, label='Strategy (3x Leverage)')
